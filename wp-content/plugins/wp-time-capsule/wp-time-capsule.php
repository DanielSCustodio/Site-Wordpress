...
Description: Backup and Staging by WP Time Capsule is an incremental automated schedule backup plugin that backups up your website to Dropbox, Google Drive and Amazon S3 on a daily basis.
Author: Revmakx
Version: 1.22.1
...
if (is_php_version_compatible_for_s3_wptc()) {
	...
	include_once ( WPTC_CLASSES_DIR . 'BackblazeFacade.php' );
}
...
function define_default_repo_const_wptc(){
	...
	if( !empty($_POST) && !empty($_POST['action']) && $_POST['action'] == 'get_backblaze_authorize_url_wptc' ){
		$config->set_option('default_repo', 'backblaze');
	}
...
function start_fresh_backup_tc_callback_wptc($type = '', $args = null, $test_connection = true, $ajax_check = true, $is_iwp = false) {
	...
	$config->set_option('site_db_name', DB_NAME);
	...
	if(!empty($args['is_migration_backup']) && $args['is_migration_backup'] != 'false'){
		$config->set_option('wptc_today_main_cycle', false);
		$config->set_option('is_migration_backup', true);
	}
	...
	global $wpdb;
	$result = $wpdb->query("TRUNCATE TABLE `" . $wpdb->base_prefix . "wptc_current_process`");

	if($result === false){

		wptc_log('', "--------truncate wptc_current_process failed so recreting table-------");

		$result2 = $wpdb->query("ALTER TABLE `" . $wpdb->base_prefix . "wptc_current_process` DISCARD TABLESPACE");
		$result3 = $wpdb->query("DROP TABLE `" . $wpdb->base_prefix . "wptc_current_process`");
...
function check_wptc_update() {
	...
	if (version_compare('1.21.21', $installed_wptc_version) > 0) {
		WPTC_Base_Factory::get('Wptc_App_Functions')->plugin_upgrades('1.21.21');
	}

	if (version_compare('1.21.23', $installed_wptc_version) > 0) {
		WPTC_Base_Factory::get('Wptc_App_Functions')->plugin_upgrades('1.21.23');
	}

	if (version_compare('1.22.0', $installed_wptc_version) > 0) {
		WPTC_Base_Factory::get('Wptc_App_Functions')->plugin_upgrades('1.22.0');
	}
	...
	if ( stripos($hook, 'wp-time-capsule') !== false 
		 ...
		 || stripos($hook, 'update-core.php') !== false ){
			...
			foreach($wp_scripts->registered as $key => $value){
				if(stripos($key, 'sweetalert') !== false && $key != 'wptc-sweetalert-js'){
					wp_deregister_script($key);
					// wp_deregister_script('swcfpc_sweetalert_js');

					wptc_log($key, "--------deregistering_other_plugin_scripts_wptc--------");
				}
			}
...
function wptc_init_actions(){
	...
	add_action('wp_ajax_get_backblaze_authorize_url_wptc', 'get_backblaze_authorize_url_wptc');
...
function get_g_drive_authorize_url_wptc() {
	...
	$config->set_option('dropbox_location', false);
...
function get_backblaze_authorize_url_wptc() {
	...
	if(!wptc_function_exist('curl_multi_exec')){
		wptc_die_with_json_encode( array('error' => 'curl_multi_exec() is not available, it is required by Backblaze, Please contact your hosting to enable it.') );
	}

	$backblaze_access_key    = $_POST['credsData']['backblaze_access_key'];
	$backblaze_secure_key    = $_POST['credsData']['backblaze_secure_key'];
	$backblaze_bucket_region = $_POST['credsData']['backblaze_bucket_region'];
	$backblaze_bucket_name   = $_POST['credsData']['backblaze_bucket_name'];

	$config = WPTC_Factory::get('config');
	$config->set_option('backblaze_access_key', $backblaze_access_key);
	$config->set_option('backblaze_secure_key', $backblaze_secure_key);
	$config->set_option('backblaze_bucket_region', $backblaze_bucket_region);
	$config->set_option('backblaze_bucket_name', $backblaze_bucket_name);
	$config->set_option('default_repo', 'backblaze');

	$result['authorize_url'] = network_admin_url() . 'admin.php?page=wp-time-capsule&cloud_auth_action=backblaze&backblaze_access_key=' . $backblaze_access_key . '&backblaze_secure_key=' . $backblaze_secure_key . '&backblaze_bucket_region=' . $backblaze_bucket_region . '&backblaze_bucket_name=' . $backblaze_bucket_name . '';
	...
	$obj = WPTC_Factory::get('BackblazeFacade');
	$obj->init();
	$response = WPTC_Factory::get('BackblazeFacade')->is_authorized_during_initial_setup();

	$config->set_option('show_user_php_error', false);

	wptc_log($result, "--------is_authorized_during_initial_setup---backblaze result-----");
...
function wptc_own_cron_status() {
	...
	// wptc_log('','--------------before do call-------------');

	$push_result = do_cron_call_wptc('status', $post_arr, 'GET');

	// wptc_log($push_result,'--------------after do call-------------');
...
function update_bulk_settings_default_flags_wptc($post_data){
	...
	// if(!empty($post_data['cloud_creds']['database_encrypt_settings'])){
	// 	WPTC_Factory::get('config')->set_option('database_encrypt_settings', $post_data['cloud_creds']['database_encrypt_settings']);
	// }
	// if(!empty($post_data['cloud_creds']['wptc_auto_update_settings'])){
	// 	WPTC_Factory::get('config')->set_option('wptc_auto_update_settings', $post_data['cloud_creds']['wptc_auto_update_settings']);
	// }
}
